...
def water_mark():
    ...
    draw.text((x, y), text, font=font, fill=(255, 0, 0), anchor='ms')
    final = Image.alpha_composite(main_image, watermark)

    f = filename.split('/')[-1]
    rand_id = random.random() * 1000000
    rand_id = int(rand_id)
    new_path = f'C:\\Users\\EMMA\\Desktop\\images{f}_watermarked_{rand_id}.png'
    final.save(new_path)
    final.show()
...
bg = tk.PhotoImage(file="./Day 84/background1.png")
